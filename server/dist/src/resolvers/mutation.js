...
export const mutationResolvers = {
    Mutation: {
        ...
        updateBooking: async (parent, args, context) => {
            const updates = {};
            if (args.player1Id)
                updates.player1 = args.player1Id;
            if (args.player2Id)
                updates.player2 = args.player2Id;
            if (args.courtId)
                updates.courtId = args.courtId;
            if (args.date)
                updates.date = args.date;
            if (args.hour)
                updates.hour = args.hour;
            if (typeof args.paid === "boolean")
                updates.paid = args.paid ? 1 : 0;
            const updatedBooking = await updateBooking(args.id, updates);
            return updatedBooking || null;
        },
        deleteBooking: async (parent, args, context) => {
            try {
        ...
        createCourt: async (parent, args, context) => {
            const newCourt = {
                id: randomUUID(), // Generate a unique UUID
                name: args.name,
                number: args.number,
        ...
        updateCourt: async (parent, args, context) => {
            const updates = {};
            if (args.name)
            ...
            if (typeof args.active === "boolean")
                updates.active = args.active ? 1 : 0;
